...
class LengthConstraint : public Element
{
private:    // types
...
public:     // methods
    ...
    double compute(Ref<Vector> g, Ref<Matrix> h) const override
    {
        ...
        HyperJet length(-m_target, m_nb_dofs);
...
public:     // python
    ...
    static void register_python(TModule& m)
    {
        ...
        using Type = LengthConstraint;
        using Holder = Pointer<Type>;
        using Base = Element;

        py::class_<Type, Base, Holder>(m, "LengthConstraint")
            .def(py::init<std::vector<Pointer<Node>>, double, double>(),
            "nodes"_a, "target"_a, "weight"_a=1.0)
        ;
    }
};
