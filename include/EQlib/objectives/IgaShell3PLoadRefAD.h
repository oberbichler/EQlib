...
class IgaShell3PLoadRefAD : public Objective
{
private:    // types
...
public:     // constructor
    ...
    double compute(Ref<Vector> g, Ref<Matrix> h) const override
    {
        ...
        const HyperJet3D ref_x = ref_geometry<2>(0);
        const HyperJet3D ref_a1 = ref_geometry<2>(1);
        const HyperJet3D ref_a2 = ref_geometry<2>(2);
        const HyperJet3D ref_a3 = ref_a1.cross(ref_a2);
        const HyperJet ref_da = ref_a3.norm();

        // actual configuration

        const Vector3D act_x = act_geometry<0>(0);
        const Vector3D act_a1 = act_geometry<0>(1);
        const Vector3D act_a2 = act_geometry<0>(2);
        const Vector3D act_a3 = act_a1.cross(act_a2);
        ...
        const HyperJet p = -sum * m_weight * ref_da.f();
...
public:     // python
    ...
    static void register_python(TModule& m)
    {
        ...
        using Type = IgaShell3PLoadRefAD;
        using Holder = Pointer<Type>;
        using Base = Objective;

        py::class_<Type, Base, Holder>(m, "IgaShell3PLoadRefAD")
